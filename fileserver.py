...
class LFTPserver:
    ...
    def handshake(self):
        global nowuploadfilename
        log_info("等待连接中")
        self.udpServer.settimeout(10)                                          #原值为3
        while True:
            try:
            ...
            if self.state == State.LISTEN:
                if message.SYN == 1:
                    ...
                    if self.fileInfo["LFTPType"] == "UPLOAD":
                        nowuploadfilename[str(self.port)]=self.fileInfo["filename"]
                        i = 0
                        c = 0
                        while c == 0:
                            i = i + 1
                            if not os.listdir("暂存文件/"):                  #如果空目录
                                onefile=open("暂存文件/abc.abc", mode='w')
                                onefile.write("123")
                                onefile.close()
                            for files in os.listdir("暂存文件/"):
                                if files == self.fileInfo["filename"]:
                                    d = 0
                                    while d < len(files):
                                        if files[d] == ')':
                                            break
                                        d = d + 1
                                    if self.fileInfo["filename"][0:d + 1] == '(' + str(i - 1) + ')':
                                        self.fileInfo["filename"] = '(' + str(i) + ')' + self.fileInfo["filename"][
                                                                                         d + 1:]
                                    else:
                                        self.fileInfo["filename"] = '(' + str(i) + ')' + self.fileInfo["filename"]
                                    c = 0
                        ...
                        self.fileInfo["filename"] = "暂存文件/" + self.fileInfo["filename"]
            ...
            while True:
                try:
                    ...
                    if (recvsize == filesize):
                        pbar.finish()
                        log_info("接收完毕，断开连接")
                        print('123' + self.fileInfo["filename"])
                        date=self.fileInfo["filename"]
                        i = 1
                        while i <= len(date):
                            if date[len(date) - i] == '/':
                                date = date[len(date) - i + 1:]
                                break
                            i = i + 1
                        global newfilename,fileuploadsituationend
                        newfilename=str(self.port)+date
                        fileuploadsituationend=1
                        print('123456:'+str(self.port))
...
def start(port,a):
    global newfilename,fileuploadsituationend,nowuploadfilename,nowdownloadclientport
    nowuploadfilename={}
    nowdownloadclientport={}
    contrl_server = LFTPserver(server_type='control', host='', port=12345, bufferSize=2048)

    contrl_server.start()
    newfilename = ''
    fileuploadsituationend = 0
